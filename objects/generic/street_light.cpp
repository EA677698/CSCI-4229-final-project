...
//    polyhedrons.push_back(pole_arm);
    ...
    for(int i = 0; i < length; i++){
        pole_arm->add_point(initial_position + Vector3(0, -(peak/2.25f) * pow(i - 1.5f, 2) + peak, (height * 1.25f) * (i/length)));
    }
    ...
    lamp_case->set_position(pole_arm->get_position().x, (height * 1.20f) - lamp_case->get_height(), pole_arm->get_depth() - lamp_case->get_depth() / 2);
